...
import br.com.dio.util.GameRepository;
...
/**
 * Importante: 
 * Para o funcionamento, insira o 
 * conteúdo entre aspas como Running Argument
 * 
 ...
 * 
 * Esta linha é responsável por definir
 * os números do jogo
 * 
 * */
public class Main {
    ...
    private static void startGame() {
        ...
        // Obtendo o Map de Spaces pronto do GameRepository
        Map<String, Space> positions = GameRepository.getParsedGameSequential();

        List<List<Space>> spaces = new ArrayList<>();
        for (int i = 0; i < BOARD_LIMIT; i++) {
            spaces.add(new ArrayList<>());
            for (int j = 0; j < BOARD_LIMIT; j++) {
                spaces.get(i).add(positions.get("%s,%s".formatted(i, j)));
            }
        }
